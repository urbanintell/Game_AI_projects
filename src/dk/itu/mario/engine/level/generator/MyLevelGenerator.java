...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		char[] result = new char[39];
		Random ran = new Random();
		for (int i = 0; i < 39; i++) {
			char curr = (char)(ran.nextInt(26) + 'A');
			result[i] = curr;
		}
		individual.setChromosome(String.valueOf(result));
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		if (population.get(0).getFitness() > 0.81) {
			decision = true;
		}
		// YOUR CODE GOES ABOVE HERE
		return decision;
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		// selected = population.subList(0, (length + 1)/2);
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		...
		ArrayList<MyDNA> temp = new ArrayList<>();
		temp.addAll(oldPopulation);
		temp.addAll(newPopulation);
		Collections.sort(temp);
		Collections.reverse(temp);
		int length = temp.size();
		finalPopulation = new ArrayList<MyDNA>(temp.subList(0, this.getPopulationSize()));
		// System.out.println(finalPopulation.size());
		// System.out.println(this.getPopulationSize());
