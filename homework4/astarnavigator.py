...
def myUpdate(nav, delta):
	...
	curr_goal = nav.agent.getMoveTarget()
	if rayTraceWorldNoEndPoints(curr_position, curr_goal, nav.world.getLines()):
		nav.agent.stop()
		nav.setPath(None)
...
def clearShot(p1, p2, worldLines, worldPoints, agent):
	### YOUR CODE GOES BELOW HERE ###
 	if helper(p1, [p2], worldLines, worldPoints, agent.getMaxRadius()*1.5):
 		return True
	### YOUR CODE GOES ABOVE HERE ###
	return False

def helper(p, nodes, worldLines, worldPoints, threshold):
	best = None
	dist = INFINITY
	for n in nodes:
