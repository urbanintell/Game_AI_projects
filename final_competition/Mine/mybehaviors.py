...
def treeSpec(agent):
	...
	#spec = [Sequence, ChaseHero, KillHero] #yeah!
	#spec = [(Selector, 0), (Retreat, 0.5, 1), [(Sequence, 2), ChaseHero, KillHero]]

	#spec = [(Selector,0), (Retreat, 0.5, 1), [(HitpointDaemon, 0.5, 2),[(Selector, 3), [(NoHeroNearbyDaemon, 4), [(Sequence,5), (ChaseMinion, 6), (KillMinion, 7)]], [(Sequence, 8), (ChaseHero, 9), (KillHero, 10)]]]]
	
	#spec = [(Sequence,0), NoTowerDaemon,GotoBase, FireBase]
	spec = [Selector, (Retreat, 0.5), [(HitpointDaemon, 0.5), [Selector, [NoEnemyNearbyDaemon, [Sequence, GotoTower, FireTower]],[NoTowerDaemon, [Sequence, GotoBase, FireBase]],[Selector, [NoHeroNearbyDaemon, [Sequence, ChaseMinion, KillMinion]],[Sequence, ChaseHero, KillHero]]]]]
	...
	def dodgeBullet(self):	
		...
		if self.agent.canDodge():
			...
			if len(bullets) > 0:
				...
				(x,y) = tuple(map(lambda x, y: x + y, self.agent.getLocation(), (vector[0]*self.agent.getMaxRadius()*3, vector[1]*self.agent.getMaxRadius()*3)))
	...
	def execute(self, delta = 0):
		...
		if self.target == None or distance(self.agent.getLocation(), self.target.getLocation()) > BIGBULLETRANGE:
			...
			if self.target == None:
				...
				print "", distance(self.agent.getLocation(), self.target.getLocation())
			#print "exec", self.id, "false"
			return False
		elif self.target.isAlive() == False:
			# succeeded
...
class NoHeroNearbyDaemon(BTNode):
	...
	def execute(self, delta = 0):
		...
		for m in enemies:
			if distance(self.agent.getLocation(), m.getLocation()) < (self.agent.getRadius()*AREAEFFECTRANGE) + (m.getRadius()):
				self.agent.areaEffect()

		if (not hero == None) and (distance(self.agent.getLocation(), hero.getLocation())) < 300:
			# fail check
			#print "exec", self.id, "fail"
			return False
		else:
...
class NoEnemyNearbyDaemon(BTNode):
