...
class Sequence(BTNode):
	...
	def execute(self, delta = 0):
		...
		num_of_children = BTNode.getNumChildren(self)
		if num_of_children > 0:
			curr_index = BTNode.getCurrentIndex(self)
			next_index = curr_index + 1
			curr_child = BTNode.getChild(self, curr_index)
			status = curr_child.execute(delta)
			if status == False:
				return False
			elif status == None:
				return None
			elif curr_index < num_of_children - 1:
				BTNode.setCurrentIndex(self, next_index)
				return None
